...
def test_build_base2_last_resort_answer_includes_olj_suggestion() -> None:
    ...
    # Bloc recette structuré : ingrédients et étapes portés par des marqueurs
    # distincts (le renderer en fait deux sections), plus une puce fourre-tout.
    assert any(s.text.startswith("§ING ") for s in out.answer_sentences)
    assert any(s.text.startswith("§STEP ") for s in out.answer_sentences)
